...
function CategoryList({ categoryList }) {
  return (
    <section className={cn(outfit.className, 'flex flex-col gap-3 w-full')}>
      <h3 className='text-primary font-bold text-2xl'>Shop by Category</h3>
      <div className='grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-1 md:gap-3'>
        {categoryList.map((cat, idx) => (
          <div
            key={idx}
            className='flex flex-col h-full bg-green-100 rounded-2xl cursor-pointer border border-primary '
          >
            <div className='p-6 flex flex-col items-center  gap-1 md:gap-3 flex-grow group hover:bg-green-300 rounded-2xl'>
              <Image
